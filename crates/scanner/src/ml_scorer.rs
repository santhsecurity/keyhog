...
pub fn score_with_config(
    ...
    thread_local! {
        // Cache last input hash (SHA-256) + score to avoid retaining raw secrets in memory.
        static LAST_SCORE: RefCell<Option<([u8; 32], f64)>> = const { RefCell::new(None) };
    }

    // Compute digest of text and context with a separating null byte to avoid collisions.
    ...
    if let Some(score) = LAST_SCORE.with(|cache| {
        cache
            .borrow()
            .as_ref()
            .and_then(|(cached_digest, cached_score)| {
                (cached_digest == &digest_arr).then_some(*cached_score)
            })
    }) {
        return score;
    }
    ...
    LAST_SCORE.with(|cache| {
        *cache.borrow_mut() = Some((digest_arr, score));
    });
    score
}
