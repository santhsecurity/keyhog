...
mod backend {
    ...
    fn init_gpu() -> Result<GpuContext, Box<dyn std::error::Error>> {
...
pub fn batch_ml_inference(candidates: &[(String, String)]) -> Vec<f64> {
    ...
    #[cfg(feature = "ml")]
    {
        // Extract features on CPU (string processing doesn't benefit from GPU)
        let features: Vec<[f32; 41]> = candidates
            .iter()
            .map(|(text, ctx)| crate::ml_scorer::compute_features_public(text, ctx))
            .collect();
        ...
        candidates
            .iter()
            .map(|(text, ctx)| crate::ml_scorer::score(text, ctx))
            .collect()
    }
