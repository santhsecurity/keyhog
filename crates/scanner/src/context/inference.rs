use super::{CodeContext, documentation::documentation_line_flags};
...
pub fn is_known_example_credential(credential: &str) -> bool {
    ...
    // 1. Ends with EXAMPLE/EXAMPLEKEY — universal documentation convention.
    if upper.ends_with("EXAMPLE") || upper.ends_with("EXAMPLEKEY") {
        return true;
    }

    // 2. Dominated by x/X filler (>75% of body is x/X).
    ...
    // 3. Sequential hex placeholder (ascending pairs like 0123456789abcdef).
    if is_hex_sequential_placeholder(credential) {
        return true;
    }

    // 4. Well-known empty-input hashes (MD5/SHA1/SHA256 of "").
    // These appear in integrity checks, not as secrets.
    if is_empty_input_hash(credential) {
        return true;
    }

    // 5. Monotonic/repetitive body after stripping known prefixes.
...
fn is_hex_sequential_placeholder(credential: &str) -> bool {
    ...
    const PREFIXES: &[&str] = &[
        ...
        "sk-proj-", "sk-ant-", "sk-", "sk_test_", "sk_live_",
        "pk_test_", "pk_live_", "rk_test_", "rk_live_",
        "AKIA", "ASIA", "xoxb-", "xoxp-", "xoxa-", "xoxr-",
        "SG.", "hf_", "npm_", "pypi-", "glpat-", "glcbt-", "glrt-",
        "dop_v1_", "eyJ", "vercel_", "sbp_", "0x",
    ];
