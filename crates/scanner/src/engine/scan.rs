...
fn generic_entropy_floor(detector_id: &str, credential_len: usize) -> f64 {
    match detector_id {
...
impl CompiledScanner {
                        ...
                        for m in &matches {
                            if let Some(ref path) = chunk.metadata.path {
                                let fragment = crate::fragment_cache::SecretFragment {
                                    ...
                                    value: m.credential.to_string(),
                                    line: m.location.line.unwrap_or(0),
                                    path: Some(path.to_string()),
                                };
                                let _reassembled = crate::fragment_cache::get_fragment_cache()
                                    .record_and_reassemble(fragment);
                                // TODO: scan reassembled candidates against the full scanner
                                // for multi-file secret detection.
                            }
                        }
                        if !matches.is_empty() {
                            return matches;
    ...
    pub fn scan_coalesced_gpu(
        ...
        let tagged = match pollster::block_on(warpstate::batch::scan_batch_gpu(matcher, items)) {
            Ok(t) => t,
            Err(e) => {
                tracing::warn!("GPU batch failed: {e}, falling back to SIMD/CPU");
                #[cfg(feature = "simd")]
                {
                    // Call the SIMD path directly — do NOT re-enter scan_coalesced
                    // which would re-check gpu_available and potentially loop.
                    return chunks.iter().map(|c| self.scan(c)).collect();
                }
                #[cfg(not(feature = "simd"))]
                return chunks.iter().map(|c| self.scan(c)).collect();
    ...
    fn process_match(
        ...
        if detector.id.starts_with("generic-") && !crate::probabilistic_gate::ProbabilisticGate::looks_promising(credential) {
            return;
        }
