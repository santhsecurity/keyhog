...
impl GitAskpassAuth {
    fn create(token: &str) -> Result<Self, SourceError> {
        ...
        // SAFETY: create the file with restricted permissions ATOMICALLY to prevent
        // a TOCTOU race where the token is briefly world-readable between write() and
        // set_permissions().
        {
            use std::io::Write;
...
fn sanitize_git_error_message(stderr: &str) -> String {
    ...
    let token_pat = TOKEN_RE
        .get_or_init(|| Regex::new(r"(ghp_|gho_|github_pat_|xoxb-|xoxp-|sk-proj-|sk_live_|sk_test_|AKIA)[A-Za-z0-9_\-]{8,}").ok());
