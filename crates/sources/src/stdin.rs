...
mod tests {
    ...
    fn read_to_string_limited_rejects_oversized_input() {
        let mut input = std::io::Cursor::new(vec![b'a'; 6]);
        let error = read_to_string_limited(&mut input, 5).unwrap_err();
        assert_eq!(error.kind(), std::io::ErrorKind::Other);
    }

    #[test]
    fn read_to_string_limited_reads_valid_utf8() {
        let mut input = std::io::Cursor::new("secret".as_bytes().to_vec());
        let text = read_to_string_limited(&mut input, 16).unwrap();
