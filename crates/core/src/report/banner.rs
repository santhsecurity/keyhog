...
/// ```rust
/// let mut out = Vec::new();
/// keyhog_core::report::banner::print_banner(&mut out, false, false, 0).unwrap();
/// assert!(!out.is_empty());
/// ```
pub fn print_banner<W: Write>(
    ...
    if color {
        ...
        writeln!(w, "    v{version} · secret scanner · {detector_count} detectors")?;
        writeln!(w, "    by santh")?;
    }
    writeln!(w)?;
...
mod tests {
    ...
    fn banner_renders_without_panic() {
        let mut buf = Vec::new();
        print_banner(&mut buf, false, false, 886)
.unwrap();
