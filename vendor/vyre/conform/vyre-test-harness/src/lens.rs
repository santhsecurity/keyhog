...
use vyre_libs::harness::{fixpoint_contract, FixpointContract, OpEntry};
...
impl LensOutcome {
    ...
    ///
    /// CRITIQUE_CONFORM_2026-04-23 H2: historically this returned true
    /// for both Pass and Skip. That made any CI gate asserting
    /// `is_ok()` pass even when 100% of ops were skipped due to a
    /// capability-filter hole — silent under-coverage. Behaviour is
    /// preserved for backwards compatibility (the semantics are "not
    /// a failure"), but [`Self::is_pass`] below MUST be used for
    /// coverage assertions.
    #[must_use]
    ...
    ///
    /// Use this in any CI gate that asserts real coverage. Unlike
    /// [`Self::is_ok`], this returns `false` for `Skip` — so a gate
    /// cannot pass because every op was silently skipped.
    #[must_use]
    pub fn is_pass(&self) -> bool {
        matches!(self, LensOutcome::Pass { .. })
    }

    /// True when the lens skipped (no real coverage produced).
    ///
    /// Callers that want to allow specific skips can use this plus a
    /// per-op allow-list instead of conflating Skip with Pass.
    #[must_use]
    pub fn is_skip(&self) -> bool {
        matches!(self, LensOutcome::Skip { .. })
    }
}
