    ...
    for entry in vyre_libs::harness::all_entries() {
        ...
        by_subsystem
            ...
            .push(OpRow {
                ...
                exempt_reason: vyre_libs::harness::universal_diff_exemption(entry.id),
            });
    }
...
fn render(subsystem: &str, rows: &[OpRow]) -> String {
    ...
    s.push_str(
        "Auto-generated by `cargo xtask catalog`. Do not hand-edit — \
    ...
    for row in rows {
        ...
        let diff = match row.exempt_reason {
            Some(reason) => format!("exempt: {reason}"),
            None => "byte-identity".to_string(),
        };
        s.push_str(&format!(
            "| `{}` | {} | {} | {} |\n",
...
fn check_against_disk(catalog: &BTreeMap<String, Vec<OpRow>>, out_dir: &std::path::Path) {
    ...
    if drift.is_empty() {
        ...
        eprintln!(
            "Fix: run `cargo xtask catalog` to regenerate, commit the updated files, then re-run --check."
        );
        std::process::exit(1);
    }
}
...
fn render_index(catalog: &BTreeMap<String, Vec<OpRow>>) -> String {
    ...
    s.push_str(
        "Auto-generated by `cargo xtask catalog`. One table per subsystem; \
